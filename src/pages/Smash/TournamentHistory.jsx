...
const TournamentHistory = () => {
    ...
    const fetchTournaments = useCallback(async () => {
        ...
        try {
            ...
            if (lastTournament) {
                tournamentsQuery = query(ref(database, 'tournaments'), orderByChild('createdAt'), endBefore(lastTournament.createdAt, lastTournament.id), limitToLast(PAGE_SIZE));
            } else {
            ...
            if (snapshot.exists()) {
                ...
                setTournaments(prev => {
                    ...
                    const newTournaments = fetchedTournaments.filter(t => !existingIds.has(t.id));
                ...
                if (fetchedTournaments.length < PAGE_SIZE) {
                    ...
                    setLastTournament(fetchedTournaments[fetchedTournaments.length - 1]);
                }
            } else {
                setHasMore(false);
            }
    ...
    }, [loading, hasMore, lastTournament]);
